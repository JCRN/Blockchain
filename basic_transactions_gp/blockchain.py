...
class Blockchain(object):
    ...
    def hash(self, block):
        ...
        string_object = json.dumps(block, sort_keys=True).encode() # encodes block from string object to unicode

        # Hash this string using sha256
        raw_hash = hashlib.sha256(string_object)
        

        hex_hash = raw_hash.hexdigest() # formats hash to hexidecimal
...
def mine(): 
    ...
    if not data['proof'] or not data['id']:
        response = {'message': 'ID and proof of work are required.'}
        ...
        if blockchain.valid_proof(block_string, data['proof']):
            ...
            response = { 'message': 'Proof of work is invalid' } 
            return jsonify(response), 406  
